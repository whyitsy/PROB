...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, criterion, data_loader_train, optimizer, device, epoch, args.nc_epoch, args.clip_max_norm, writer, args)
        if writer is not None:
            for k, v in train_stats.items():
                if isinstance(v, (int, float)):
                    writer.add_scalar(f'epoch_train/{k}', v, epoch)
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.lr_drop == 0 or (epoch % args.eval_every == 0 or epoch == 0 or epoch == 1 or (args.epochs-epoch)<1):
                ...
                if writer is not None:
                    for key, val in test_stats["metrics"].items():
