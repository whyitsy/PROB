#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Chapter 3 objectness-focused sweeps on top of C3 best + cls soft attenuation.
# P1: obj_loss_coef
# P2: uod_pseudo_obj_loss_coef
# P3: uod_obj_neg_loss_coef
# P4: uod_cls_soft_attn_alpha
# -----------------------------------------------------------------------------
set -x
set -euo pipefail

BASE_EXP_DIR="${1:-/mnt/data/kky/output/PROB/exps/MOWODB/UOD_ABL_T1_CH3_OBJECTNESS_PARAMS}"
shift $(( $# > 0 ? 1 : 0 )) || true
PY_ARGS="${@:1}"
