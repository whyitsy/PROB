...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, **kwargs):
        ...
        logit_scale = kwargs.get('logit_scale', None)
        ...
        if self.two_stage:
            ...
            # [NEW: OVD Two-Stage 核心逻辑] =================================
            # 1. 提取 Encoder 的密集特征并投影
            enc_outputs_proj = self.decoder.proj_head[self.decoder.num_layers](output_memory)
            
            # 2. 动态点乘计算所有密集位置的分类 Logits
            if clip_text_features is not None:
                proj_feat_norm = F.normalize(enc_outputs_proj, p=2, dim=-1)
                text_feat_norm = F.normalize(clip_text_features, p=2, dim=-1).to(proj_feat_norm.dtype)
                
                scale = logit_scale.exp() if logit_scale is not None else 1.0
                enc_outputs_class = scale * proj_feat_norm @ text_feat_norm.t()
            else:
                # 兼容防崩溃
                enc_outputs_class = torch.zeros((bs, output_memory.shape[1], 81), device=output_memory.device)
            # ===============================================================
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, decoder_layer, num_layers, return_intermediate=False):
        ...
        # [NEW] 替换原来的 self.class_embed = None
        self.proj_head = None
