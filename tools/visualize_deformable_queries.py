...
def plot_query_sampling(image_np, records, query_kind, query_index, scores, output_path):
    ...
    if not ordered:
        return
    ncols = len(ordered)
    figure, axes = plt.subplots(1, ncols, figsize=(4.4 * ncols, 4.6))
    if ncols == 1:
        axes = [axes]
    h, w = image_np.shape[:2]
    for axis, (layer_id, record) in zip(axes, ordered):
        ...
        for lvl in range(heads_mean_locations.shape[0]):
            ...
            ys = pts[:, 1] * h
            sizes = 20.0 + 200.0 * ws
            axis.scatter(xs, ys, s=sizes, c=color, alpha=0.75, edgecolors='black', linewidths=0.4)
    ...
    oscore = scores['obj_prob'][query_index]
    uscore = scores['unknown_prob'][query_index]
    kscore = scores['max_known'][query_index]
    figure.suptitle(
        f'Query {query_index} | type={query_kind} | select_score={qs:.3f} | obj={oscore:.3f} | unk={uscore:.3f} | max_known={kscore:.3f}',
        y=0.98,
        fontsize=12,
    )
    figure.tight_layout()
...
def build_parser():
    parser = argparse.ArgumentParser('Standalone Deformable DETR query visualization', parents=[get_args_parser()])
...
def main(parsed_args):
    ...
    for query_kind, query_index in selected:
        output_path = out_dir / f'{query_kind}_query_{int(query_index):03d}.png'
        plot_query_sampling(base_image, records, query_kind, int(query_index), scores, output_path)

    print(f'Saved query visualizations to: {out_dir}')
