...
def build_parser():
    parser = argparse.ArgumentParser('SVG-first ODQE gate visualization', parents=[get_args_parser()])
    ...
    parser.add_argument('--output_subdir', default='infer/odqe_gates', type=str)
    return parser
...
def main(parsed_args):
    ...
    save_gate_statistics_csv(selected, gate_records, scores, out_dir / 'selected_query_gate_statistics.csv')
    plot_selected_query_gate_overview(selected, gate_records, out_dir / 'selected_query_gate_mean_heatmap.svg')

    for query_kind, query_index in selected:
        plot_query_gate_curve(query_kind, int(query_index), gate_records, scores, out_dir / f'{query_kind}_query_{int(query_index):03d}_gate_curve.svg')
        plot_query_gate_heatmap(query_kind, int(query_index), gate_records, out_dir / f'{query_kind}_query_{int(query_index):03d}_gate_heatmap.svg')

    print(f'Saved SVG ODQE gate visualizations to: {out_dir}')
