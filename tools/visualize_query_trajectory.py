...
LAYER_COLORS = ['#E53935', '#1E88E5', '#43A047', '#FB8C00', '#8E24AA', '#00897B', '#00897B', '#6D4C41']
KNOWN_COLOR = '#00A65A'
...
def save_trajectory_csv(selected, layer_scores, gate_records, image_hw, output_path):
...
def plot_image_trajectory(ax, image_np, abs_boxes_per_layer, query_kind):
    ...
    for layer_id, box in enumerate(abs_boxes_per_layer):
        ...
        rect = Rectangle((x1, y1), max(1.0, x2 - x1), max(1.0, y2 - y1), fill=False, linewidth=1.7, edgecolor=color, alpha=0.95)
        ax.add_patch(rect)
        cx = 0.5 * (x1 + x2)
        cy = 0.5 * (y1 + y2)
        centers_x.append(cx)
        centers_y.append(cy)
        ax.scatter(cx, cy, s=36, c=color, edgecolors='black', linewidths=0.4)
        ax.text(cx + 2, cy + 2, str(layer_id), color='white', fontsize=8, bbox=dict(facecolor='black', alpha=0.55, pad=1))
    if len(centers_x) >= 2:
        ax.plot(centers_x, centers_y, linewidth=1.8, color=KNOWN_COLOR if query_kind == 'known' else UNKNOWN_COLOR, alpha=0.9)
    ax.set_title('Query box trajectory on image')


def plot_box_geometry(ax, layers, abs_boxes_per_layer):
...
def plot_score_trajectory(ax, layer_scores, query_index, query_kind):
...
def plot_layer_class_trace(ax, layer_scores, query_index):
    ...
    for x, y, label in zip(layer_ids, vals, labels):
        ax.text(x, y + 0.02, f'c{label}', ha='center', va='bottom', fontsize=8)
    ax.set_xlabel('decoder layer')
...
def plot_query_trajectory_panel(query_kind, query_index, image_np, layer_scores, gate_records, image_hw, output_path):
    ...
    plot_image_trajectory(axes[0, 0], image_np, abs_boxes_per_layer, query_kind)
    plot_box_geometry(axes[0, 1], layer_scores, abs_boxes_per_layer)
    plot_score_trajectory(axes[0, 2], layer_scores, query_index, query_kind)
    plot_gate_trajectory(axes[1, 0], gate_records, query_index)
    plot_layer_class_trace(axes[1, 1], layer_scores, query_index)
    ...
    axes[1, 2].set_title('Trajectory summary')

    fig.suptitle(f'Cross-layer trajectory of query {int(query_index)} ({query_kind})', y=0.99, fontsize=14)
...
def plot_selected_overview(selected, layer_scores, gate_records, output_path):
    ...
    fig, ax = plt.subplots(figsize=(1.1 * len(layer_ids) + 2.6, 0.52 * len(row_labels) + 2.4))
    ...
    ax.set_title('Unknownness x effective-gate overview')
...
def build_parser():
    parser = argparse.ArgumentParser('Standalone cross-layer query trajectory visualization', parents=[get_args_parser()])
...
def main(parsed_args):
    ...
    save_trajectory_csv(selected, layer_scores, gate_records, image_hw, out_dir / 'selected_query_trajectory.csv')
    plot_selected_overview(selected, layer_scores, gate_records, out_dir / 'selected_query_unknown_gate_overview.png')
    ...
    for query_kind, query_index in selected:
        plot_query_trajectory_panel(
            ...
            gate_records,
            image_hw,
            out_dir / f'{query_kind}_query_{int(query_index):03d}_trajectory.png'
        )

    print(f'Saved cross-layer query trajectories to: {out_dir}')
