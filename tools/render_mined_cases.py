...
from tools.visualize_deformable_queries import (
    to_numpy_image,
    draw_gt_boxes,
    compute_query_scores,
    plot_query_sampling,
)
from tools.visualize_odqe_gates import (
    ODQEGateRecorder,
    ordered_gate_records,
    plot_query_gate_curve,
    plot_query_gate_heatmap,
)
from tools.visualize_odqe_query_mechanism import (
    MSDeformAttnRecorder,
    ordered_attention_records,
    plot_joint_panel,
)
from tools.visualize_query_trajectory import (
...
def render_case(entry, case_dir, base_image, scores, attn_records, gate_records, layer_scores, image_hw, render_modes):
    ...
    if 'sampling' in render_modes:
        output_path = case_dir / f'{query_kind}_query_{query_index:03d}_sampling.png'
    ...
    if 'gate' in render_modes and gate_records:
        curve_path = case_dir / f'{query_kind}_query_{query_index:03d}_gate_curve.png'
        heatmap_path = case_dir / f'{query_kind}_query_{query_index:03d}_gate_heatmap.png'
    ...
    if 'joint' in render_modes and attn_records and gate_records:
        output_path = case_dir / f'{query_kind}_query_{query_index:03d}_joint_mechanism.png'
    ...
    if 'trajectory' in render_modes:
        output_path = case_dir / f'{query_kind}_query_{query_index:03d}_trajectory.png'
...
def build_parser():
    parser = argparse.ArgumentParser('Batch renderer for mined representative cases', parents=[get_args_parser()])
...
def main(parsed_args):
    ...
    for sample_index, sample_entries in grouped_entries.items():
        ...
        base_image_path = image_dir / 'image_with_gt.png'
        if not base_image_path.exists():
            Image.fromarray(base_image).save(base_image_path)
    ...
    print(f'Saved rendered mined cases to: {out_dir}')
