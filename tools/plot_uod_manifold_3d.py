...
def plot_feature_manifold_3d(npz_path, output_dir, max_points):
    ...
    fig.tight_layout()
    fig.savefig(output_dir / 'feature_manifold_3d.png', bbox_inches='tight')
    plt.close(fig)
...
def plot_score_space_3d(stats, output_dir, max_points):
    ...
    fig.tight_layout()
    fig.savefig(output_dir / 'score_space_3d.png', bbox_inches='tight')
    plt.close(fig)
...
def plot_score_slices(stats, output_dir, max_points):
    ...
    for axis, ((x_idx, y_idx), x_label, y_label, title) in zip(axes, slice_specs):
        for group_index, (group_name, color) in enumerate(zip(GROUP_NAMES, GROUP_COLORS)):
            mask = groups == group_index
            if np.any(mask):
                axis.scatter(features[mask, x_idx], features[mask, y_idx], s=8, alpha=0.58, c=color, label=group_name)
    ...
    fig.tight_layout()
    fig.savefig(output_dir / 'score_space_slices.png', bbox_inches='tight')
    plt.close(fig)


def build_parser():
    parser = argparse.ArgumentParser('Standalone 3D manifold and decision boundary plotting')
...
def main(args):
    ...
    print(f'Saved 3D plots to: {output_dir}')
