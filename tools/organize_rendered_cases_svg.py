...
def build_parser():
    parser = argparse.ArgumentParser('SVG-first atlas organizer for rendered thesis figures')
...
def main(args):
    ...
    for category in all_categories:
        for mode in ['sampling', 'gate', 'joint', 'trajectory']:
            ...
            if not items:
                continue
            board_path = boards_dir / f'{category}_{mode}_sheet.svg'
            title = f'{category} | {mode} gallery'
            write_gallery_svg(items, board_path, title=title, mode=mode)
    ...
    for chapter_name, spec in chapter_specs.items():
        for mode in spec['modes']:
            ...
            if not items:
                continue
            board_path = boards_dir / f'{chapter_name}_{mode}_sheet.svg'
    ...
    print(f'Saved SVG figure atlas to: {output_dir}')
